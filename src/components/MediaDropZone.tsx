...
const MediaDropZone: React.FC<MediaDropZoneProps> = ({
  ...
  return (
    ...
    <div
      className={`relative min-h-[120px] md:min-h-[200px] border-2 border-dashed rounded-xl p-4 md:p-10 text-center cursor-pointer transition-all duration-300 flex items-center justify-center ${
        isDragOver
          ...
          : isUploading
          ? 'border-green-500 dark:border-green-400 bg-green-50 dark:bg-green-900/20 cursor-not-allowed'
          : 'border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-800 hover:border-blue-500 dark:hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-blue-900/20'
      }`}
    ...
    >
      ...
      {isUploading ? (
        <div className="flex flex-col items-center gap-3">
          <div className="w-8 h-8 border-3 border-gray-200 dark:border-gray-700 border-t-blue-500 dark:border-t-blue-400 rounded-full animate-spin"></div>
          <p className="text-gray-700 dark:text-gray-300 font-medium">Uploading media...</p>
          {uploadProgress > 0 && (
        ...
        <div className="space-y-2 md:space-y-4">
          ...
          <div>
            <p className="text-sm md:text-lg font-semibold text-gray-700 dark:text-gray-300 mb-1 md:mb-2">
    ...
    {/* Error Message */}
    {error && (
      <div className="flex items-center gap-2 p-3 mt-4 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg text-red-800 dark:text-red-200">
        <span>⚠️</span>
        <span className="flex-1">{error}</span>
        <button onClick={clearError} className="text-red-800 dark:text-red-200 hover:text-red-900 dark:hover:text-red-100 text-lg font-bold">
          ×
        </button>
      </div>
    )}
    ...
    {selectedFiles.length > 0 && (
      <div className="mt-6 p-5 bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
        ...
        <div className="space-y-3">
          {selectedFiles.map((file, index) => (
            <div key={index} className="p-3 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
              <div className="flex items-center justify-between p-2 md:p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600">
                <div className="flex-1 min-w-0">
                  <p className="text-xs md:text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{file.name}</p>
              ...
              <div className="flex items-center gap-1 md:gap-2 ml-2 md:ml-4">
                <input
                  ...
                  className="px-2 md:px-3 py-1 text-xs md:text-sm border border-gray-300 dark:border-gray-600 rounded focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 w-20 md:w-auto bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400"
                  disabled={isUploading}
                />
                <button
                  onClick={() => removeFile(index)}
                  disabled={isUploading}
                  className="p-1 md:p-2 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-full transition-colors disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation"
                  title="Remove file"
                >
